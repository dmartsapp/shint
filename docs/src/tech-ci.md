...
<svg viewBox="0 0 700 300" role="img" aria-label="Workflow graph: one tag push starts five workflows">
  ...
  <text class="sub" x="75" y="164" text-anchor="middle">v4.1.0</text>
...
| Compute tags | From the tag `v4.1.0`: `v4.1.0`, `4.1.0`, `4.1`, `4`, and `latest`. |
...
| Part | What it shows |
|---|---|
| Headline and colour | `shint v4.1.0 is released` on green when everything passed; `the release pipeline failed` on red; `still running` on amber if the wait ran out |
...
gh release view v4.1.0                      # the release and its assets
```
