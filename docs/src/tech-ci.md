...
<svg viewBox="0 0 700 300" role="img" aria-label="Workflow graph: one tag push starts five workflows">
  ...
  <text class="sub" x="75" y="164" text-anchor="middle">v4.2.0</text>
...
The whole of `make check` takes about three and a half minutes on a runner (roughly: tools to install 1 minute, `go vet` for four operating systems 45 s, the race-detector tests 70 s, the battery 50 s). The maintainer runs it anyway, on the commit, before merging - so CI does not have to repeat all of it, provided it can **check** that the run happened and **cannot be given a forgery**.
...
| Compute tags | From the tag `v4.2.0`: `v4.2.0`, `4.2.0`, `4.2`, `4`, and `latest`. |
...
| Part | What it shows |
|---|---|
| Headline and colour | `shint v4.2.0 is released` on green when everything passed; `the release pipeline failed` on red; `still running` on amber if the wait ran out |
...
gh release view v4.2.0                      # the release and its assets
```
