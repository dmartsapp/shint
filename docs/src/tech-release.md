...
|---|---|---|
| `go build` from a clone | `4.1.0` | The default in `main.go` (bump it in the release commit) |
| `make <target>` | `<tag-or-dev>-<commit date as ddmmyyyyHHMMSS>` | `Makefile` (`git tag --contains`, `git show --format=%cd`) |
| Release binary (CI) | `v4.1.0/<full commit sha>/<UTC build time>` | `build.yaml` (`-X main.Version=${{ github.ref_name }}/${{ github.sha }}/$DT`) |
| Docker image | `v4.1.0` | `Dockerfile` (`ARG VERSION`, passed as `VERSION=<tag>`; `dev` when built locally) |
