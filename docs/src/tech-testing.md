...
Workflows run only for a release tag, so a mistake in one would first show itself on release day. Two things stand in for that. `bash .github/scripts/test-verify-release-tag.sh` runs the guard script against a fake `gh` (no network) and checks: a name that is not `vX.Y.Z` is refused **without** calling GitHub; `identical` and `behind` pass; `ahead` and `diverged` are refused; an API error refuses; a missing input is an error. And the script was run once against the real repository - v4.0.3's commit passes, a release-branch tip is refused as `ahead` - with the same `gh api` call the workflow makes. The workflow files themselves are checked with [actionlint](https://github.com/rhysd/actionlint) (run without `shellcheck`, so the result is the same on a runner that has it and a laptop that does not; [issue #51](https://github.com/dmartsapp/shint/issues/51) tracks the findings it would report), and by the [trigger-rule check](tech-ci.md#the-workflow-trigger-rule), which fails if any workflow could be started by anything but a release tag on `main` (or, for a future workflow, a push to `main` that ignores `.github/**`). These, and the tests for the release preflight (`release-check.sh`, in throw-away git repositories), the failure-issue script and the checksum script (`write-checksums.sh`: format, an independent SHA-256, the platform's own `-c` verifier, a tampered file), run as `make workflows`, part of `make check`.

The [Slack notifier](tech-ci.md#slack-notification) has its own tests, `test_notify_slack.py`: the message in every state, the waiting, the posting against a local server, and that no webhook URL is ever committed. They run as part of `make workflows` too.
