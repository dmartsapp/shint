...
func TestWebHandlerBytesIncludeHeaders(t *testing.T) {
	...
	check := func(t *testing.T, rs *rawServer, tlsConfig *tls.Config) {
		...
		out := captureStdout(t, func() {
			WebHandler(&jsonOutput, 1, 0, &throttle, 5, rs.url, "POST", body, headers, false, tlsConfig)
		})
...
func TestWebHandlerLogLine(t *testing.T) {
	...
	out := captureStdout(t, func() {
		WebHandler(&jsonOutput, 1, 0, &throttle, 5, rs.url, "GET", "", nil, false, nil)
	})
...
func TestWebHandlerReusedConnectionCountsPerRequest(t *testing.T) {
	...
	jsonOutput, throttle := true, false
	out := captureStdout(t, func() {
		WebHandler(&jsonOutput, 2, 150, &throttle, 5, serverURL, "GET", "", nil, false, nil)
	})
	stats := webStatsFromJSON(t, out)
	if len(stats) != 2 {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			out := captureStdout(t, func() {
				...
				WebHandler(&webJSON, 1, 0, &throttle, 10, serverURL, tc.method, body, []string{"X-Test: 1"}, false, nil)
				<-done
			})
...
func TestWebHandlerBytesCoverEveryRedirectHop(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			jsonOutput, throttle := true, false
			out := captureStdout(t, func() {
				WebHandler(&jsonOutput, 1, 0, &throttle, 5, serverURL, "GET", "", nil, false, nil)
			})
			stats := webStatsFromJSON(t, out)
