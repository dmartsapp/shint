...
| **v4.2.1** | Released Sep 23 | Release binaries now cover AIX, illumos, and 32-bit and Power Linux (18 binaries in all); `dns`'s JSON field `server` is renamed `nameserver`, `ip` prints one line and one flat object per interface, and the Vulnerability Check workflow no longer fights itself over the Go module cache |
